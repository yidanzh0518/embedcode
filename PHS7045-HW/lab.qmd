...
myfun2(1) #this also work! lazy evaluation 
```
...
myreg <- function(...,outputname){
  ...
  assign(
    ...
    fit,envir = .GlobalEnv #using the assignment function and output in the global environment
  )
  invisiable(NULL) # don't print anything on the screen 
}

#myreg <- lm(my~wt,data=data,outputname=mymodel)
```
...
library(R.utils)
# Download the data
met <- fread("/Users/yidanzhang/Downloads/2024-Fall/PHS-7045 R/met_all.gz")
...
mergedata <- merge(stations,met,by.x="USAF",by.y ="USAFID",
                               ,all.y = TRUE)
```
