...
#'
#' #' @details
#' This function is designed for HPC environments using the `slurmR` package for job scheduling.
#' It splits the data by `id`, processes each subset in parallel across multiple Slurm jobs,
#' and computes co-occurrences for the `code` values within the specified `window`.
...
cooccur_hpc <- function(
  ...
  if (!is.null(pll_njobs)) {
    # If the number of jobs (pll_njobs) is defined, use Slurm to distribute tasks across parallel jobs
    ...
    # Stop execution if pll_njobs is not defined
    stop("Please define the number of jobs (pll_njobs) and cores (mc.cores) to use.")
  }
